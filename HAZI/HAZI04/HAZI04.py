...
def math_passed_count(df_data):
    return len(df_data[df_data["math score"]>49])
...
def add_grade(df_data):
    grade = (df['math score']+df['reading score']+df['writing score'])/300
    df['grade'] = grade
    return df_data
...
def math_bar_plot(df_data):
    ...
    arr = df.groupby("gender", group_keys=False).mean()
    print(arr)
